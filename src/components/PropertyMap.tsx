...
export function PropertyMap({ properties, center = [20.5937, 78.9629], zoom = 5 }: PropertyMapProps) {
  // If no center provided, calculate center from properties
  const mapCenter = properties.length > 0 
    ? [properties[0].latitude, properties[0].longitude] as [number, number]
    : center

  return (
    <div className="h-full w-full rounded-xl overflow-hidden border shadow-sm z-0">
      <MapContainer
        center={mapCenter}
        zoom={properties.length > 0 ? 12 : zoom}
        style={{ height: '100%', width: '100%' }}
      >
        <ChangeView center={mapCenter} zoom={properties.length > 0 ? 12 : zoom} />
        <TileLayer
